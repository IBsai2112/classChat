...
module.exports = function GetUTCEpochNanoseconds(
	...
	var date = MakeDay(year, month - 1, day); 
	var time = MakeTime(hour, minute, second, millisecond); 
	var ms = MakeDate(date, time); 
	if (!isInteger(ms)) {
		throw new $TypeError('Assertion failed: `ms` from MakeDate is not an integral Number'); 
	}

	if (!$BigInt) {
		throw new $SyntaxError('BigInts are not supported in this environment');
	}
	return $BigInt((ms * 1e6) + (microsecond * 1e3) + nanosecond); 
};
