...
module.exports = function MakeMatchIndicesIndexPairArray(S, indices, groupNames, hasGroups) {
	...
	var A = ArrayCreate(n); 
	var groups = hasGroups ? OrdinaryObjectCreate(null) : void undefined; 
	CreateDataPropertyOrThrow(A, 'groups', groups); 

	for (var i = 0; i < n; i += 1) { 
		var matchIndices = indices[i]; 
		
		var matchIndexPair = typeof matchIndices !== 'undefined' ? GetMatchIndexPair(S, matchIndices) : void undefined; 
		CreateDataPropertyOrThrow(A, ToString(i), matchIndexPair); 
